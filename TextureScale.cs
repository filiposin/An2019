...
namespace LunarCatsStudio.SuperCombiner
{
    public class TextureScale
    {
        ...
        private static void ThreadedScale(Texture2D tex, int newWidth, int newHeight, bool useBilinear)
        {
            ...
            w = tex.width;
            w2 = newWidth;
            int num = Mathf.Min(Mathf.Max(SystemInfo.processorCount, 1), newHeight);
            ...
            // Создаем новую текстуру и устанавливаем пиксели
            Texture2D newTex = new Texture2D(newWidth, newHeight);
            newTex.SetPixels(newColors);
            newTex.Apply();

            // Присваиваем новую текстуру оригинальной текстуре
            tex = newTex; // Это не изменяет оригинальную текстуру в Unity, вам нужно будет сохранить ее куда-то или использовать по-другому.
